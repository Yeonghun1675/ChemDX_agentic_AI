...
{context}
- Fuzzy match features and coerce numeric values.
- Provide concise evidence-backed summaries.
"""
...
def analyze_trend(x_feature: str, y_feature: str, file_path: Optional[str] = None, filters: Optional[str] = None) -> str:
    ...
    if _df_cache is None or (file_path and file_path != _path_cache):
        status = load_matdx_db(file_path)
    ...
    lines: List[str] = []
    lines.append(f"MatDX trend of '{y_feature}' vs '{x_feature}'")
